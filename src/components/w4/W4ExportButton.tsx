...
import { W4Report } from '@/types/database'
...
export function W4ExportButton({ report, fileName = 'w4-report' }: Props) {
  const [isExporting, setIsExporting] = useState(false)
  const [showDropdown, setShowDropdown] = useState(false)

  const generateMarkdown = (): string => {
    const { client_name, rep_name, company_name, overall_performance, phases, what_done_right, areas_for_improvement, weakest_elements, coaching_recommendations, rank_assessment, quick_wins } = report

    let md = `# COMPREHENSIVE CALL REPORT

**Client:** ${client_name || 'Unknown'}
**Representative:** ${rep_name || 'Unknown'}
...
| Metric | Value |
|--------|-------|
| **Total Score** | ${overall_performance.total_score} / 100 |
...
`
    ...
    const phaseNames = { why: 'WHY', what: 'WHAT', who: 'WHO', when: 'WHEN' }
    
    for (const phaseKey of phaseOrder) {
      const phase = phases[phaseKey]
      if (!phase) continue

      md += `## PHASE: ${phaseNames[phaseKey]} (Max: ${phase.max_score})
...
`
      for (const checkpoint of phase.checkpoints || []) {
        ...
        md += `| ${checkpoint.name} | ${checkpoint.score} | ${checkpoint.max_score} | ${justification} |
`
      }
...
| **Overall Total** | **${overall_performance.total_score}** | **100** |
...
`
    ...
    if (coaching_recommendations && Object.keys(coaching_recommendations).length > 0) {
      md += `## COACHING RECOMMENDATIONS\n\n`
      for (const [topic, advice] of Object.entries(coaching_recommendations)) {
        if (advice) {
    ...
    if (quick_wins?.length > 0) {
      ...
      for (let i = 0; i < quick_wins.length; i++) {
        ...
        md += `${i + 1}. **${win.title}** – ${win.action} (Worth ${win.points_worth} points)\n`
      }
    }
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      // Header
      addText(`Client: ${client_name || 'Unknown'}`)
      addText(`Representative: ${rep_name || 'Unknown'}`)
      addText(`Company: ${company_name || 'Unknown'}`)
      
      // Overall Performance
      addSection('OVERALL PERFORMANCE')
      addText(`Total Score: ${overall_performance.total_score} / 100`, 12, true)
      ...
      for (const phaseKey of phaseOrder) {
        ...
        addSection(`PHASE: ${phaseNames[phaseKey]} (${phase.score}/${phase.max_score})`)
        
        for (const checkpoint of phase.checkpoints || []) {
          addText(`• ${checkpoint.name}: ${checkpoint.score}/${checkpoint.max_score}`, 10, true)
      ...
      addText(`TOTAL: ${overall_performance.total_score}/100`, 11, true)
      ...
      if (coaching_recommendations && Object.keys(coaching_recommendations).length > 0) {
        ...
        for (const [topic, advice] of Object.entries(coaching_recommendations)) {
          if (advice) {
            const formattedTopic = topic.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
            addText(formattedTopic, 11, true)
            addText(advice)
            y += 3
          }
        }
      ...
      if (rank_assessment) {
        ...
        y += 2
        addText('Next Level Requirements:', 10, true)
        addText(rank_assessment.next_level_requirements)
      }
      ...
      if (quick_wins?.length > 0) {
        ...
        for (let i = 0; i < quick_wins.length; i++) {
          const win = quick_wins[i]
          addText(`${i + 1}. ${win.title} (${win.points_worth} pts)`, 10, true)
          addText(`   ${win.action}`)
        }
      }
  ...
  const handleExportDocx = async () => {
    ...
    try {
      ...
      // Header info
      addParagraph(`Client: ${client_name || 'Unknown'}`, true)
      addParagraph(`Representative: ${rep_name || 'Unknown'}`)
      addParagraph(`Company: ${company_name || 'Unknown'}`)
      
      // Overall Performance
      addHeading('OVERALL PERFORMANCE', HeadingLevel.HEADING_1)
      addParagraph(`Total Score: ${overall_performance.total_score} / 100`, true)
      ...
      for (const phaseKey of phaseOrder) {
        ...
        addHeading(`PHASE: ${phaseNames[phaseKey]} (Max: ${phase.max_score})`, HeadingLevel.HEADING_1)
        ...
        for (const checkpoint of phase.checkpoints || []) {
          tableRows.push(new TableRow({
            children: [
              ...
              new TableCell({ children: [new Paragraph({ text: String(checkpoint.max_score), alignment: AlignmentType.CENTER })] }),
      ...
      addParagraph(`Overall Total: ${overall_performance.total_score} / 100`, true)
      ...
      if (coaching_recommendations && Object.keys(coaching_recommendations).length > 0) {
        ...
        for (const [topic, advice] of Object.entries(coaching_recommendations)) {
          if (advice) {
            const formattedTopic = topic.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
            addParagraph(formattedTopic, true)
            addParagraph(advice)
          }
        }
      }
      ...
      if (rank_assessment) {
        ...
        addParagraph('Next Level Requirements:', true)
        addParagraph(rank_assessment.next_level_requirements)
      }
      ...
      if (quick_wins?.length > 0) {
        ...
        for (let i = 0; i < quick_wins.length; i++) {
          const win = quick_wins[i]
          addParagraph(`${i + 1}. ${win.title} (${win.points_worth} pts)`, true)
          addParagraph(`   ${win.action}`)
        }
      }
